...
**Выключить второй фактор на проде нельзя.** Раньше здесь стояла обратная
инструкция — «на время приёмки можно», — и вышло ровно то, чего и следовало
ожидать: выключили, приёмка кончилась, дети в базе появились, а строка
в `.env` осталась. Владелец и администратор несколько недель заходили по
одному паролю, и заметили это случайно.

Поэтому `TWO_FACTOR_ENABLED` в проде больше не действует: настройки
продакшна включают второй фактор жёстко, что бы ни стояло в `.env`. Снять
его можно только правкой кода — осознанно и с оглядкой, а не строкой, про
которую через месяц никто не вспомнит.

В разработке выключатель работает по-прежнему. Но и там, как только
в базе появляется хотя бы один ученик, `manage.py check` останавливается
с ошибкой `accounts.E001`: проверка обычная, а не только для `--deploy`,
потому что `--deploy` никто не запускает.

Если доступ ко второму фактору потерян — не выключайте его, а сбросьте
устройство конкретному человеку командой `reset_two_factor` выше: он
подключит новое при следующем входе.
