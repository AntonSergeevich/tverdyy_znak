...
Нужен, когда Actions недоступны или изменение на сервере, а не в коде:

```powershell
.\deploy\deploy.ps1
```

Скрипт пушит текущую ветку и запускает на сервере тот же
`remote-deploy.sh`. Тесты локально не гоняет — их место в CI, где база
всегда есть.
