...
занятия его детей, ученику — свои.
...
Чтобы занятия появились в самом кабинете, сетку надо загрузить. Порядок:

1. В Яндекс.Документах: «Файл → Скачать → CSV».
2. Привести колонки к виду из [schedule.example.csv](schedule.example.csv):
   `day,time,subject,group,teacher,room,duration`. Названия предметов и групп
   должны совпадать с теми, что заведены в журнале.
3. Положить файл на сервер и загрузить в нужный модуль:

```powershell
# Код в контейнер приезжает из образа, папка проекта внутрь не смонтирована,
# поэтому файл кладём в два шага: на сервер, а оттуда — внутрь контейнера.
scp raspisanie.csv tz@85.198.66.41:/srv/tverdyy-znak/raspisanie.csv
ssh tz@85.198.66.41 "cd /srv/tverdyy-znak && docker compose cp raspisanie.csv web:/tmp/raspisanie.csv"
ssh tz@85.198.66.41 "cd /srv/tverdyy-znak && docker compose exec -T web python manage.py import_schedule /tmp/raspisanie.csv --module 1 --dry-run"
```
