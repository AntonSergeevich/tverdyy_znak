...
приводит справочники к коду и дёргает `/healthz`.

Справочники — это модули учебного года, список предметов и реквизиты ИП:
они объявлены в коде источником истины, поэтому выкат приводит базу
к ним. Тексты первого экрана, цены и ссылку на расписание деплой не
трогает: их правят в админке, и возвращать их назад он не должен.
Вернуть значения из кода намеренно — `setup_client_data --force`.

Тесты скрипт запускает питоном из `.venv` проекта. Если локального
PostgreSQL нет, он это распознаёт, называет причину и идёт дальше:
отсутствие базы на ноутбуке — не красный прогон, и выкат из-за него
не останавливается. А вот настоящее падение тестов останавливает.

Полезные ключи:

```powershell
.\deploy\deploy.ps1 -SkipPush          # выкатить то, что уже в origin
.\deploy\deploy.ps1 -SkipTests         # не запускать тесты вовсе
