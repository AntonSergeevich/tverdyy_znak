...
# Сайт отвечает — но это ещё не всё приложение. Фоновые процессы
# (планировщик, воркер) страниц не отдают, и проверка выше их не видит.
# Планировщик так и падал по кругу четыре недели: сайт работал, выкат
# рапортовал успех, а автоудаление персональных данных не запускалось.
#
# Ждём немного: упавший контейнер успевает перезапуститься и снова
# упасть, и тогда его состояние — «Restarting».
sleep 10
restarting="$(docker compose ps --status restarting --format '{{.Service}}' || true)"
if [ -n "$restarting" ]; then
    echo "" >&2
    echo "  Падают и перезапускаются по кругу: $restarting" >&2
    for service in $restarting; do
        echo "" >&2
...
echo "  Все контейнеры работают, по кругу не перезапускается ни один."
